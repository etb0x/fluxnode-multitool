...
if ! [[ -z $1 ]]; then
    if [[ $BRANCH_ALREADY_REFERENCED != '1' ]]; then
        ...
        bash -i <(curl -s https://raw.githubusercontent.com/etb0x/fluxnode-multitool/$ROOT_BRANCH/multitoolbox_testnet.sh) $ROOT_BRANCH
        unset ROOT_BRANCH
...
bash -i <(curl -s https://raw.githubusercontent.com/etb0x/fluxnode-multitool/${ROOT_BRANCH}/install_pro_testnet.sh)
